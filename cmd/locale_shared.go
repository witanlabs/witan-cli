...
// resolveLocaleSimple is a simple version that doesn't validate and just returns the first available value.
func resolveLocaleSimple(flagValue string) string {
	if flagValue != "" {
		return flagValue
	}

	if v := os.Getenv("WITAN_LOCALE"); v != "" {
		return v
	}
