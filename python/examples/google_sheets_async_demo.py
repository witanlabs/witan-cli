#!/usr/bin/env python3
"""
Async demo of the Witan Python SDK for Google Sheets.
...
Usage:
    cd python && python3 examples/google_sheets_async_demo.py gs://YOUR_SHEET_REF
    cd python && python3 examples/google_sheets_async_demo.py --create --title "Witan demo"
"""
...
from witan import AsyncGoogleSheet, Regex, WitanRPCError, is_google_auth_required


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Witan async Google Sheets SDK demo")
    parser.add_argument(
        "ref",
        nargs="?",
    ...
    parser.add_argument(
        "--title",
        default="Witan SDK async demo",
        help="Title when using --create (default: %(default)s).",
    )
    return parser.parse_args()


def open_sheet(args: argparse.Namespace) -> AsyncGoogleSheet:
    if args.create:
        return AsyncGoogleSheet.create(title=args.title)
    if not args.ref:
        print("error: ref is required unless --create is set", file=sys.stderr)
        sys.exit(2)
    return AsyncGoogleSheet(args.ref)


async def run_demo(args: argparse.Namespace) -> None:
    async with open_sheet(args) as sheet:
        if sheet.is_create:
        ...
        print("\n=== Reading data ===")
        print(await sheet.read_range_tsv("Sheet1!A1:B4"))

        avg = await sheet.read_cell("Sheet1!B4")
        print(f"\nAverage cell: value={avg['value']!r} formula={avg.get('formula')!r}")

        print("\n=== Search ===")
        found = await sheet.find_cells("Alice")
        print(f"Found Alice at: {[c['address'] for c in found]}")

        numbers = await sheet.find_cells(Regex(r"^\d+$"))
        ...
        desc = await sheet.describe_sheet("Sheet1")
