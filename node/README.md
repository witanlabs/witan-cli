...
| Python | Node.js |
|--------|---------|
| `wb.list_sheets()` | `wb.listSheets()` |
| `wb.read_cell()` | `wb.readCell()` |
| `wb.find_and_replace()` | `wb.findAndReplace()` |

All method names follow JavaScript camelCase conventions.

### Async Only

The Node.js SDK is async-only. Python provides both sync (`Workbook`) and async (`AsyncWorkbook`), but Node.js only provides async:
...
Python uses seconds, Node.js uses milliseconds:

```python
# Python
wb = Workbook('report.xlsx', request_timeout=30)  # 30 seconds
```
...
Python uses the `Regex` class, Node.js uses native `RegExp`:

```python
# Python
from witan import Regex
cells = wb.find_cells(Regex(r'\d{3}-\d{4}'))
```
...
Node.js uses a static factory method instead of a constructor:
...
```python
# Python
from witan import __version__
```

```typescript
// Node.js
import { version } from 'witan';
```
