...
Then verify, as always: `xlsx_calc` with `verify: true` (`errors` must be empty), `xlsx_lint` (resolve or knowingly accept every finding), and `xlsx_render` on `Model!A1:D8` to confirm the look.
