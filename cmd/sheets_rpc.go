...
func openSheetsRPCSession(ctx context.Context, c *client.Client, params sheetsRPCConnectParams) (*sheetsRPCSession, error) {
	...
	if !initResp.Ok {
		conn.CloseNow()
		return nil, sheetsRPCInitFailure(&initResp, params.SpreadsheetID)
	}
	if initResp.SpreadsheetID == "" {
		conn.CloseNow()
		return nil, fmt.Errorf("init response missing spreadsheet_id")
	}
...
// sheetsRPCInitFailure handles a failed RPC init frame. Authorization-related
// codes are surfaced as agent-friendly output plus an exit-code-3 ExitError;
// the spreadsheet id is the one we sent in init (the server no longer echoes
// it). Branch on the frame code, not the WebSocket close code. Other failures
// fall back to the formatted message error.
func sheetsRPCInitFailure(resp *sheetsRPCInitResponse, spreadsheetID string) error {
	switch resp.Code {
	case "needs_file_authorization", "google_auth_required":
		return emitSheetsAuthError(resp.Code, spreadsheetID, resp.Message, gsheetsJSONOutput)
	default:
		return formatSheetsRPCInitError(resp)
	}
}
