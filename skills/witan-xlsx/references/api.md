...
`witan xlsx exec` runs sandboxed JavaScript against a workbook opened server-side. Your script gets two globals: `xlsx` (this API) and `wb` (the workbook handle, passed first to every call). Top-level `await` is supported; `import` is not.
