...
async function openSheet(args: ReturnType<typeof parseArgs>): Promise<GoogleSheet> {
  ...
  await ensureAuthorized(args.ref);
  return GoogleSheet.open(args.ref);
}

/**
 * Make sure the app is authorized for this sheet, handing the picker URL off to
 * a human if not. Sheets you create are authorized automatically.
 */
async function ensureAuthorized(ref: string): Promise<void> {
  const info = await GoogleSheet.authorizeUrl(ref);
  if (info.authorized) {
    return;
  }
  console.error(
  ...
  await GoogleSheet.waitUntilAuthorized(ref);
}
...
async function main() {
  ...
  try {
    ...
    if (err instanceof WitanRPCError) {
      ...
      } else if (isNeedsFileAuthorization(err)) {
        console.error(
          'This sheet is not authorized. Run:\n' + `  witan gsheets authorize ${args.ref}`
        );
      } else {
